...
Intuition: Alice is a featured app because they act as an automated market maker. Because the Allce app consistently submits transactions that compose multiple assets and application features, it has an ActivityWeight of 1.25, while Bank1 and Bank2 have the default ActivityWeight of 1. 
...
* Alice gets the following `per_app_traffic_weight` for Node 1:
  ...
  Multiplying by the app's `ActivityWeight`, its resulting weight becomes:
`per_ap_traffic_weight * ActivityWEight = 12 * 1.25 = 15`
* Bank1 gets the following `per_app_traffic_weight` for Node 2:
  `per_app_traffic_weight = (4 * 25) / (25 * 2) = 2 kB`
  While Alice receives an additional 25%: 
  `per_app_traffic_weight = (4 * 25) / (25 * 2) = 2kB`
  `per_app_traffic_weight * ActivityWeight = 2 * 1.25 = 2.5`
